...
def drop_path(x, drop_prob: float = 0., training: bool = False):
    ...
    # 这里shape的类型是tuple，第一个元素是x.shape[0],后面有（x的维度-1）个1 ->: (n,1,1,1,...)
...
class PatchEmbed(nn.Module):
    ...
    # VIT_base模型的嵌入维度是768，如果是其它大的模型的话嵌入深度会更大
    def __init__(self, img_size=224, patch_size=16, in_c=3, embed_dim=EMBEDDING_DIM, norm_layer=None):
        ...
        # 用二维卷积做的嵌入,输入3通道,输出768通道;步长=patch_size大小；
        # 维度从（224*224*3） -> （14*14*768）
...
# 用于实现Multi-head self-attention的模块
class Attention(nn.Module):
    def __init__(self,
        ...
        # 多头的情况下,每个小头的维度=输入维度/head的个数
        head_dim = dim // num_heads

        # Q乘以K的转置之后,需要除以一个根号dk,理论里面的公式有介绍
        self.scale = qk_scale or head_dim ** -0.5 #这相当于是负的0.5次方

        # 通过一个全连接层生成三个Q,K,V矩阵(有些代码是用三个全连接层分别生成)
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)

        # 这里似乎可以做一个channel attention， 因为每个头的信息是没有交互的

        # drop的值是传入的值
        self.attn_drop = nn.Dropout(attn_drop_ratio)

        # 这个全连接层用于将多个head的结果拿来拼接
